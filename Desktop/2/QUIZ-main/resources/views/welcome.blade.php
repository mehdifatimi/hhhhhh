...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    border-radius: 0.25rem;">{{$question->question}}</div><br><br>
                            ...
                            @else
                                ...
                                Bienvenue dans le système de quiz.
                                </span><br><br>
                                <span class="badge badge-success" style="color: white; font-size: 140%;">
                                Il vous sera posé un total de  {{request()->session()->get('questions')}} questions
                                </span><br><br>
                                <span class="badge badge-success" style="color: white; font-size: 140%;">
                                Vous aurez {{request()->session()->get('time')}} secondes pour répondre .
                                </span><br><br>
                                <span class="badge badge-dark" style="color: white; font-size: 140%;">
                                Bon Chance!
                                </span><br><br><br>
                                <a href="?page=1" class="btn btn-primary btn-lg btn-block">Start</a>
                            @endif
                            @else
                            ...
                            <form name="check-first" id="check-first" method="post" action="{{url('check-first')}}">
                                <center>
                                    @csrf
                                    <label for="time"><b>Temps de réponse par question (15 à 120 secondes).</b></label>
                                    <input type="text" class="form-control" name="time" value="{{ old('time') }}">
                                    <br>
                                    <label for="questions"><b>Nombre total de questions (1 à 10).</b></label>
                ...
                @if(request()->session()->has('time') && request()->session()->has('questions'))
                    <center><br>
                    <span class="badge badge-info" style="color: white; font-size: 130%;">{{request()->session()->get('time')}} secondes par question</span>
                    <span class="badge badge-info" style="color: white; font-size: 130%;">Nombre total de questions: {{request()->session()->get('questions')}}</span>
                    <br><br>
            ...
            if(sessionStorage.getItem({{$question->id}} + "out") != "out"){
                var downloadTimer = setInterval(function(){
                    if(sessionStorage.getItem({{$question->id}} + "time") <= 0){
                        ...
                        document.getElementById("countdown").innerHTML = sessionStorage.getItem({{$question->id}} + "time") + " secondes restantes";
                    }
            ...
            document.addEventListener("DOMContentLoaded", function(event) {
                ...
                btn.addEventListener("click", () => {
                    ...
                    }else if(sessionStorage.getItem({{$question->id}}) !== selectedSize)
                    {
                        var sH = sessionStorage.getItem({{$question->id}});
                        if(sH == "A" || sH == "B" || sH == "C" || sH == "D"){
                            alert("La réponse que vous avez choisie pour cette question était : "+ sessionStorage.getItem({{$question->id}}) +". Elle ne peut pas être modifiée une fois le temps écoulé ou une fois la réponse sélectionnée..")
                            }else{
                            alert("Vous n'avez pas répondu à cette question. Elle ne peut pas être modifiée une fois le temps écoulé.")
                        }
                    }
                });
